...
public class PagerFlowLayout extends FlowLayout {
    ...
    private void nextPageWithoutLayout() {
        ...
        if (mAdapter != null) {
            int pageViewSize = mPagerViews.size();
            int viewTypeCount = mAdapter.getViewTypeCount();
            if (pageViewSize > 0 && viewTypeCount > 0) {
                for (ViewWrapper viewWrapper : mPagerViews) {
                    viewCache.cache(viewWrapper.type, viewWrapper.view, calculateCacheLength(pageViewSize, viewTypeCount));
                }
            }
        }
        mPagerViews.clear();
        ...
        while (layoutHelper.measure(child.view) && mPagerViews.size() < childViewCount) {
...
//        }
    ...
    private int calculateCacheLength(int pageViewSize, int viewTypeCount) {
        return Math.round(1.0f * pageViewSize / viewTypeCount);
    }
